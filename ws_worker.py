...
BROWSER_API_URL_TEMPLATE = "http://localhost:49156/profiles/open?windowId={}"
...
class VideoToTextWorker:
    ...
    def get_cdp_ws_url(self, window_id: int = 1) -> str | None:
        api_url = BROWSER_API_URL_TEMPLATE.format(window_id)
        try:
    ...
    async def start_browser_cdp(self, window_id: int | None = None):
        """通过 CDP 接管已打开的浏览器窗口（复用登录态）"""
        if window_id is None:
            # 使用进程 ID 作为 window_id 避免多窗口冲突
            import os
            window_id = os.getpid() % 10 + 1  # 限制在 1-10 范围
        
        ws_url = self.get_cdp_ws_url(window_id)
        if not ws_url:
    ...
    async def start_browser(self, data_dir: str | None = None, cdp_api_url: str | None = None):
        ...
        # 带重试：TargetClosedError 通常是另一个 Chrome 占用了同一 data_dir
        max_retries = 3
        for attempt in range(1, max_retries + 1):
            try:
                ...
                print(f"[Browser] 第 {attempt}/{max_retries} 次启动失败: {e}")
                if attempt >= max_retries:
                    raise RuntimeError(f"浏览器启动失败（已重试 {max_retries} 次）: {e}") from e
                print(f"[Browser] 可能有另一个 Chrome 实例占用 {data_dir}，{3 * attempt}秒后重试...")
    ...
    async def chatglm_worker_loop(self, worker_id: int, page):
        ...
        while True:
            ...
            try:
                ...
                # 无论成功或失败，都标记任务完成并从队列移除
                self.chatglm_queue.task_done()
                print(f"[ChatGLMWorker-{worker_id}] 任务已完成并移除 key={key}，队列剩余: {self.chatglm_queue.qsize()}")
                
                # 任务失败后尝试重置页面状态，避免影响下一个任务
                try:
                    await page.goto(CHATGLM_VIDEO_URL, wait_until="networkidle", timeout=30_000)
                except Exception as reset_e:
                    print(f"[ChatGLMWorker-{worker_id}] 重置页面失败: {reset_e}")
...
if __name__ == "__main__":
    ...
    # 抑制 Windows 上 asyncio 子进程传输层 __del__ 的 "I/O operation on closed pipe" 噪音警告
