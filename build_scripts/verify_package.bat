...
echo.
echo [检查8] 验证打包配置...
if exist "build_scripts\setup_zhimengai.iss" (
    echo   ✓ build_scripts\setup_zhimengai.iss
) else (
    echo   ✗ build_scripts\setup_zhimengai.iss 缺失
    set /a ERROR_COUNT+=1
)

echo.
echo [检查9] 验证编译工具...
if exist "build_scripts\build_pyc_for_package.py" (
    echo   ✓ build_scripts\build_pyc_for_package.py
) else (
    echo   ✗ build_scripts\build_pyc_for_package.py 缺失
    set /a ERROR_COUNT+=1
)
...
if %ERROR_COUNT%==0 (
    ...
    echo.
    echo 运行以下命令开始打包：
    echo   build_scripts\build_package.bat
) else (
