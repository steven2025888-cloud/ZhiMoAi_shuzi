...
export default {
	...
	computed: {
		validImage() {
			return this.image && this.image !== '' && this.image !== 'true'
		},
		cellStyle() {
			return {
				width: this.width + 'px',
		...
		imgStyle() {
			const s = Math.min(this.width, this.height) - this.padding * 2
			return {
				width: s + 'px',
				height: s + 'px',
				borderRadius: this.radius + 'rpx'
	...
	methods: {
		onDelete() {
			this.$emit('delete', { index: Number(this.index) })
		}
	}
}
</script>
...
.z-sort-item__cross--a {
	transform: rotate(45deg);
}

.z-sort-item__cross--b {
	transform: rotate(-45deg);
}
